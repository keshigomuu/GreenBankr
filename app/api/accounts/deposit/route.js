// app/api/accounts/deposit/route.js
...
export async function POST(req) {
  try {
    ...
    const url = `${base.replace(
      /\/$/,
      ""
    )}/account/${encodeURIComponent(customerId)}/DepositCash`;
    ...
    const upstreamRes = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        narrative: narrative || "Deposit via GreenBankr",
      }),
    });
    ...
    if (!upstreamRes.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: msg || `DepositCash error (${upstreamRes.status})`,
        },
