...
/**
 * Main Donations API wrapper used on the SERVER (in API routes).
 * Do NOT import this directly in client components/hooks.
 */
export const DonationsAPI = {
  // ============================================================
  // MakeDonation (POST /NewDonation)
  // ============================================================
  async addDonation({ customerId, amount, orgId, customerDepositAccount }) {
    try {
  ...
  async getByCustomer(customerId) {
    ...
    return list.map((d) => ({
      ...
      id:
        d.Id ??
        d.id ??
        `${paddedId}-${d.Org_ID ?? d.orgId ?? "org"}-${
          d.DateOfTxn ?? d.date ?? Date.now()
        }`,
    }));
  },
};
