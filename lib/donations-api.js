...
export const DonationsAPI = {
  async addDonation({ customerId, amount, orgId }) {
    ...
    const payload = {
      CustomerId: paddedId,
      Cust_Acct_Id: "0000005982",
      Recieving_Acct_Id: "0000005953",
    ...
    console.log("[donations] POST", NEW_DONATION_URL, "payload:", payload);

    const res = await fetch(NEW_DONATION_URL, { method: "POST", headers, body: JSON.stringify(payload) });
    const data = await safeJson(res);

    if (!res.ok) {
      console.error("[donations] NewDonation failed:", res.status, data);
      throw new Error(data?.error || data?.message || `NewDonation failed (${res.status})`);
    }

    console.log("[donations] NewDonation OK:", data);
    return data || { donationId: data?.donationId };
  },
