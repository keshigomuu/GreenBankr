...
export async function createCustomer(formData) {
  ...
  const payload = {
    ...
    phoneAreaCode: String(formData.phoneAreaCode || ""),
    phoneLocalNumber: String(formData.phoneLocalNumber || ""),
    positionTitle: String(formData.positionTitle || ""),
    employerName: String(formData.employerName || ""),
    officeAddress1: String(formData.officeAddress1 || ""),
    officeAddress2: String(formData.officeAddress2 || ""),
    officeAddress3: String(formData.officeAddress3 || ""),
    officeContactNumber: String(formData.officeContactNumber || ""),
    officeContactNumberExt: String(formData.officeContactNumberExt || ""),
    preferredUserld: String(formData.preferredUserld || ""), // NOTE: lowercase "l"
    currency: String(formData.currency || ""),
    password: String(formData.password || ""),

    // Numbers
    yearOfService: Number(formData.yearOfService) || 0,
    annualSalary: Number(formData.annualSalary) || 50000,

    // Booleans
    workingInSingapore: Boolean(formData.workingInSingapore),
    createDepositAccount: Boolean(formData.createDepositAccount),
