...
export class OrganisationsAPI {
  ...
  static async getEnvOrganisations() {
    const url = `https://kxl.outsystemscloud.com/CharityInformation/rest/OrganisationService/GetEnvOrganisations`;

    const res = await fetch(url, { cache: "no-store" });
    const data = await res.json();

    if (!res.ok) {
      throw new Error(`GetEnvOrganisations failed: ${res.status}`);
    }
...
export async function getOrganisationPreference(customerId) {
  const base = process.env.DON_PREF_BASE_URL;
  const apiKey = process.env.DON_PREF_API_KEY;

  const res = await fetch(`${base}/Preference/${customerId}`, {
    headers: {
